...
import { getEntryAttachmentBuffer, getEntryAttachmentById, softDeleteEntryAttachment } from "@/lib/entryAttachments";
...
export async function GET(_request: Request, { params }: RouteContext) {
  ...
  const attachment = await getEntryAttachmentById(attachmentId);
  if (!attachment || attachment.entry_id !== id || attachment.deleted_at) {
    return NextResponse.json({ error: "Not found" }, { status: 404 });
  }

  const bytes = await getEntryAttachmentBuffer(attachment);
  if (!bytes) {
...
export async function DELETE(_request: Request, { params }: RouteContext) {
  ...
  await softDeleteEntryAttachment(attachmentId);
